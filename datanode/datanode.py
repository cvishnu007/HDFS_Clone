...
# ---------------- CONFIG ----------------
DATANODE_NAME = os.environ.get("DATANODE_NAME", "datanode1")
DATANODE_HOST = os.environ.get("DATANODE_HOST", "0.0.0.0")
DATANODE_PORT = int(os.environ.get("DATANODE_PORT", "5001"))

NAMENODE_HOST = os.environ.get("NAMENODE_HOST", "127.0.0.1")
NAMENODE_HEARTBEAT_PORT = int(os.environ.get("NAMENODE_HEARTBEAT_PORT", "6000"))
...
def recv_all(conn,timeout=30):  # NEW
    ...
    while True:
        try:
            ...
            print(f"[{DATANODE_NAME}] ⏳ recv_all timeout after {len(buf)} chunks")
            break
    return b"".join(buf)
...
def handle_conn(conn, addr):
    try:
        ...
        if action == "store":
            fname = req["filename"]
            blob = base64.b64decode(req["content_b64"])
            ...
            if not os.path.exists(path):
                ...
                conn.sendall(json.dumps({"status": "ok", "content_b64": content_b64}).encode())
        elif action == "delete":
            fname = req["filename"]
            path = os.path.join(DATA_DIR, fname)
